...
using BellucSketch.Api.Middleware;
using BellucSketch.Application.Abstractions;
using BellucSketch.Application.Common;
using BellucSketch.Application.Projetos;
using BellucSketch.Infrastructure;
using BellucSketch.Infrastructure.Import;
using BellucSketch.Infrastructure.Persistence;
using BellucSketch.Infrastructure.Repositories;
using BellucSketch.Infrastructure.Storage;
...
// --- Persistência ---
builder.Services.AddDbContext<BellucSketchDbContext>(options =>
    // Chave de config "Camdas" (não "BellucSketch") mantida de propósito: é o nome da variável de
    // ambiente ConnectionStrings__Camdas já configurada manualmente (sync: false) no painel do
    // Render, com a connection string real do Supabase — renomear aqui sem renomear lá (que exige
    // ação manual no painel, fora do alcance do render.yaml) faria a Api subir sem achar o banco.
...
// --- CORS ---
// Necessário para o visualizador web (BellucSketch.Web, Blazor WebAssembly) chamar esta Api de outra
...
const string PoliticaCorsWeb = "BellucSketchWeb";
...
using (var escopo = app.Services.CreateScope())
    escopo.ServiceProvider.GetRequiredService<BellucSketchDbContext>().Database.Migrate();
...
// ApplicationDisplayVersion do BellucSketch.Mobile.csproj e pelo VerificadorAtualizacaoGitHub do app,
